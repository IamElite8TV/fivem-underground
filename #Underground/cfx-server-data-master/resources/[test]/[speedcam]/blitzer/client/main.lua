...
local extraZonePrice20 = 300 -- THIS IS THE EXTRA COST IF 20 KM/H ABOVE LIMIT (REQUIRES "useBilling" to be set to true)
local extraZonePrice30 = 600 -- THIS IS THE EXTRA COST IF 30 KM/H ABOVE LIMIT (REQUIRES "useBilling" to be set to true)
...
local blips = {
	...
	-- 120KM/H ZONES
	{title="Blitzer (120KM/H)", colour=1, id=1, x = 1584.9281, y = -993.4557, z = 59.3923}, -- 120KM/H ZONE
	{title="Blitzer (120KM/H)", colour=1, id=1, x = 2442.2006, y = -134.6004, z = 88.7765}, -- 120KM/H ZONE
	{title="Blitzer (120KM/H)", colour=1, id=1, x = 2871.7951, y = 3540.5795, z = 53.0930} -- 120KM/H ZONE
}
								...
								-- VEHICLES ABOVE ARE BLACKLISTED
									...
									if alertPolice == true then
										if SpeedKM > alertSpeed then
											...
											TriggerServerEvent('esx_phone:send', 'police', ' Jemand fuhr an der Radarkamera vorbei, über ' .. alertSpeed.. ' KMH', true, {x =x, y =y, z =z})
										end
									end
									-- ALERT POLICE (END)								
									...
									TriggerEvent("pNotify:SendNotification", {text = "Der Blitzer hat dich in einer 60er-Zone erwischt! Deine Geschwindigkeit: " .. math.floor(SpeedKM) .. " KM/H", type = "error", timeout = 5000, layout = "centerLeft"})
									...
									if useBilling == true then
										...
										TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(PlayerId()), 'society_police', 'Speedcamera (60KM/H) - Deine Geschwindigkeit: ' .. math.floor(SpeedKM) .. ' KM/H - ', finalBillingPrice) -- Sends a bill from the police
									else
						...
						if (GetPedInVehicleSeat(playerCar, -1) == playerPed) then					
							if hasBeenCaught == false then
								...
								-- VEHICLES ABOVE ARE BLACKLISTED
									...
									TriggerEvent("pNotify:SendNotification", {text = "Der Blitzer hat dich in einer 80er-Zone erwischt! Deine Geschwindigkeit: " .. math.floor(SpeedKM) .. " KM/H", type = "error", timeout = 5000, layout = "centerLeft"})
									...
									if useBilling == true then
										...
										TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(PlayerId()), 'society_police', 'Speedcamera (80KM/H) - Deine Geschwindigkeit: ' .. math.floor(SpeedKM) .. ' KM/H - ', finalBillingPrice) -- Sends a bill from the police
									else
						...
						if (GetPedInVehicleSeat(playerCar, -1) == playerPed) then 
							if hasBeenCaught == false then
								...
								-- VEHICLES ABOVE ARE BLACKLISTED
									...
									TriggerEvent("pNotify:SendNotification", {text = "Der Blitzer hat dich in einer 120er-Zone erwischt! Deine Geschwindigkeit: " .. math.floor(SpeedKM) .. " KM/H", type = "error", timeout = 5000, layout = "centerLeft"})
									...
									if useBilling == true then
										...
										TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(PlayerId()), 'society_police', 'Speedcamera (120KM/H) - Deine Geschwindigkeit: ' .. math.floor(SpeedKM) .. ' KM/H - ', finalBillingPrice) -- Sends a bill from the police
									else
