...
local weapons = {
    {["name"] = "pistol", ["item"] = "WEAPON_PISTOL", ['ammo'] = 150, ["price"] = 5000},
    {["name"] = "beacon", ["item"] = "WEAPON_FLAREGUN", ['ammo'] = 20, ["price"] = 3000},

    {["name"] = "knife", ["item"] = "WEAPON_KNIFE", ['ammo'] = 1, ["price"] = 200},
    {["name"] = "baseball-bat", ["item"] = "WEAPON_BAT", ['ammo'] = 1, ["price"] = 50},
}
    ...
    for key, value in pairs(weapons) do
        if value["name"] == data.name then
            ESX.TriggerServerCallback('ps_waffenladen:canAfford', function(bool)
                if bool then
                    ESX.ShowNotification(("Du hast dir eine Waffe für $%s gekauft"):format(value["price"]))
                else
                    ESX.ShowNotification("Du kannst dir diese Waffe nicht leisten!")
                end
	        end, value)
        end
    end    
...
Citizen.CreateThread(function()
    while true do
        ...
        for key, value in pairs(laden) do
            ...
            if dist <= 2.0 then
                ESX.ShowHelpNotification("Drücke ~INPUT_CONTEXT~ um auf den Waffenhandel zuzugreifen")
