...
AddEventHandler('rs_crafting:CraftingSuccess', function(CraftItem)
    ...
    if CraftItem == "weapon_assaultrifle" or CraftItem == "weapon_gusenberg" then
    ...
    TriggerClientEvent('esx:showNotification', src, "Du hast eine "..item.label.."~w~ erstellt!")
end)
-- Fail server event
...
AddEventHandler('rs_crafting:CraftingFailed', function(CraftItem)
    ...
    TriggerClientEvent('esx:showNotification', src, "~r~Du konntest keine ~b~ Erstellen"..item.label)
end)
