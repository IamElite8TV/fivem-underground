...
RegisterNetEvent("togglesduty")
AddEventHandler("togglesduty", function()
    local playerPed = PlayerPedId()
    if sduty then
        sduty = false
        TriggerEvent("chatMessage", "^8Server ^7| Du bist jetzt nicht als Teamler im Dienst. Schreibe ^8 /asduty ^7 um wieder in den Dienst zu gehen.")
        ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin)
            TriggerEvent('skinchanger:loadSkin', skin)
        end)
    else
        sduty = true
        TriggerEvent("chatMessage", "^8Server ^7| Du bist jetzt als Teamler im Dienst. Schreibe ^8 /asduty ^7 um den Dienst zu beenden.")
        setUniform()
    end 
end)