...
AddEventHandler('KorioZ-PersonalMenu:Boss_promouvoirplayer2', function(target)
	...
	local sourceXPlayer = ESX.GetPlayerFromId(_source)
	local targetXPlayer = ESX.GetPlayerFromId(target)
	local maximumgrade = tonumber(getMaximumGrade(sourceXPlayer.job3.name)) -1

	if (targetXPlayer.job3.grade == maximumgrade) then
		TriggerClientEvent('esx:showNotification', _source, "Vous devez demander une autorisation du ~r~Gouvernement~w~.")
	else
		if (sourceXPlayer.job3.name == targetXPlayer.job3.name) then
			local grade2 = tonumber(targetXPlayer.job3.grade) + 1
			local job3 = targetXPlayer.job3.name

			targetXPlayer.setjob3(job3, grade2)
...
AddEventHandler('KorioZ-PersonalMenu:Boss_destituerplayer2', function(target)
	...
	if (targetXPlayer.job3.grade == 0) then
		TriggerClientEvent('esx:showNotification', _source, "Vous ne pouvez pas plus ~r~rétrograder~w~ davantage.")
	else
		if (sourceXPlayer.job3.name == targetXPlayer.job3.name) then
			local grade2 = tonumber(targetXPlayer.job3.grade) - 1
...
AddEventHandler('KorioZ-PersonalMenu:Boss_recruterplayer2', function(target, job3, grade2)
	local _source = source
