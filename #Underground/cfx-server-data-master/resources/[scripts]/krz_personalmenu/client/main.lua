...
function RefreshMoney2()
	if ESX.PlayerData.job3 ~= nil and ESX.PlayerData.job3.grade_name == 'boss' then
		ESX.TriggerServerCallback('esx_society:getSocietyMoney', function(money)
...
AddEventHandler('esx_addonaccount:setMoney', function(society, money)
	...
	if ESX.PlayerData.job3 ~= nil and ESX.PlayerData.job3.grade_name == 'boss' and 'society_' .. ESX.PlayerData.job3.name == society then
		UpdateSociety2Money(money)
	end
end)
...
function AddMenuWalletMenu(menu)
	...
	local walletjob3 = nil

	if Config.doublejob then
		walletjob3 = NativeUI.CreateItem(_U('wallet_job3_button', ESX.PlayerData.job3.label, ESX.PlayerData.job3.grade_label), "")
		walletmenu.SubMenu:AddItem(walletjob3)
	end
...
function AddMenuBossMenu2(menu)
	bossMenu2 = _menuPool:AddSubMenu(menu, _U('bossmanagement2_title', ESX.PlayerData.job3.label))
		...
		if item == recruter2Item then
			if ESX.PlayerData.job3.grade_name == 'boss' then
				...
				if personalmenu.closestPlayer == -1 or personalmenu.closestDistance > 3.0 then
					...
					TriggerServerEvent('KorioZ-PersonalMenu:Boss_recruterplayer2', GetPlayerServerId(personalmenu.closestPlayer), ESX.PlayerData.job3.name, 0)
				end
			else
				ESX.ShowNotification(_U('missing_rights'))
			end
