...
Citizen.CreateThread(function()
	...
	pickupStation = { --Set the car rental locaitons here
		{x = -522.27, y = -254.84, z = 34.67},		--Airport car rental place
		--{x = 1677.2429199219, y = 2658.6179199219, z = 45.560031890869}

	}
	
	dropoffStation = { --Set the car dropoff locations here
		{x = -903.59967041016, y = -2309.703125, z = 5.7090353965759}, --Airport car rental place
		{x = 241.49575805664, y = -755.84222412109, z = 29.82596206665}, -- PV At Legion SQ
	...
	WarMenu.CreateMenu('carRental', 'Fahrzeug Vermietung')
	WarMenu.CreateSubMenu('closeMenu', 'carRental', 'Bist du dir sicher?')
	WarMenu.CreateSubMenu('carPicker', 'carRental', 'Pick a car | 1 day about ' .. rentalTimer ..  ' minutes')
	WarMenu.CreateSubMenu('carInsurance', 'carRental', 'Willst du dir eine Versicherung holen?')
	WarMenu.CreateMenu('carReturn', 'Fahrzeug Vermietung Rückgabe')
	WarMenu.SetSubTitle('carReturn', 'Willst du das Fahrzeug zurückgeben?') 
	WarMenu.CreateMenu('arrestCheck', 'Fahrzeug Vermietung')
	WarMenu.SetSubTitle('arrestCheck', 'Bist du gerade verhaftet?')
		...
		if not isBlipCreated then 
			for _, v in pairs(pickupStation) do
				pickupBlip = AddBlipForCoord(v.x, v.y, v.z)
      			...
      			AddTextComponentString("Fahrzeug Vermietung")
			...
			for _, v in pairs(dropoffStation) do
				pickupBlip = AddBlipForCoord(v.x, v.y, v.z)
      			...
      			SetBlipAsShortRange(pickupBlip, true)
	  			BeginTextCommandSetBlipName("STRING")
      			AddTextComponentString("Fahrzeug Rueckgabe")
      			EndTextCommandSetBlipName(pickupBlip)
			end
			isBlipCreated = true
		else
